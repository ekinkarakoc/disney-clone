  ...
  return (
    <div className="flex gap-2 md:gap-5 p-2 px-5 md:px-16">
      {productionHouseList.map((item, index) => {
        return (
          <div
            className="border-[2px] border-gray-600 rounded-lg hover:scale-110 transition-all duration-300 ease-in-out cursor-pointer relative shadow-xl shadow-gray-800 bg-[#424242]"
            key={index}
          >
            <video
              src={item.video}
            ...
            />
            <img src={item.image} alt="" className="w-full z-10" />
          </div>
        );
      })}
    </div>
